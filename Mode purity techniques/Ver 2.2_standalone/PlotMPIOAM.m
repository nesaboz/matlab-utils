...
%        legend('OAM', 'TE + TM','LP_{01}')
    ...
    case 'Time',
        ...
        plot(x(index1:index2) - x(index1), MPI.a1(index1:index2),'b')
        plot(x(index1:index2) - x(index1), MPI.a2(index1:index2),'r')

%        plot(x(index1:index2) - x(index1), MPI.a(index1:index2), 'r')
        plot(x(index1:index2) - x(index1), MPI.b(index1:index2),'g')
        plot(x(index1:index2) - x(index1), MPI.c(index1:index2),'m')
        
        legend('OAM^{+}_{ }','OAM^{-}_{ }', 'TE_{01} + TM_{01}','LP_{01}')
%        legend('OAM', 'TE_{01} + TM_{01}','LP_{01}')
        xlabel('Time (min)');
        ylabel('Relative mode power (dB)');
        temp = ParsePathname(pathname);
