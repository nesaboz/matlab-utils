function [maxima, angles, ringPoints] = FindIntensityAroundRingDetailedR0Fixed(im, showCrosses, xcm, ycm, radius, theta)
% function [maxima, angles, ringPoints] = FindIntensityAroundRingTheBest(im, showCrosses (optional))
% 
...
for i = 1:M
    ...
    ringPoints(i,1) = round(xcm - radius*sind(theta(i)));
    ringPoints(i,2) = round(ycm + radius*cosd(theta(i)));
    
    maxima(i) = im(ringPoints(i,1), ringPoints(i,2));
...
if showCrosses
    ...
    for i = 1:M
        PutCross(ringPoints(i,2), ringPoints(i,1), [3 3] ,gcf, [1 0 0]);
    end
end
