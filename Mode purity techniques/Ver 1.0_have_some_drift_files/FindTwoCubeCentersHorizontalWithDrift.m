function [xCenters, yCenters, radius] = FindTwoCubeCentersHorizontalWithDrift(cube,index)
...
Idc1 = cube(x(1) + (-radius:radius),y(1) + (-radius:radius),index);
Shim(Idc1, 1, 'chose center point');
...
Idc2 = cube(x(3) + (-radius:radius),y(3) + (-radius:radius),index);
Shim(Idc2, 1,'chose center point');
